...
export function GetAllUsers() {
	return async (dispatch: Dispatch) => {
		...
		try {
			const { data } =
				await PrivateAxios.get<GetUsersResponse>(
					GET_ALL_USERS,
				)
...
export function UpdateInfo(_id: string, newInfo: any) {
	return async function (dispatch: Dispatch) {
		...
		try {
			const { data } =
				await PrivateAxios.put<UpdateUserInfoResponse>(
					`${UPDATE_USER}/${_id}`,
					newInfo,
				)

			dispatch({
				type: ActionTypes.SET_IS_USER_NOT_UPDATING,
			})
...
export function Login(
	...
	return async function (dispatch: Dispatch) {
		...
		try {
			const { data } =
				await ClientAxios.post<LoginUserResponse>(
					LOGIN_USER,
					{
...
export function ClearUserError(dispatch: Dispatch) {
	return dispatch<ClearUserErrorAction>({
