...
const FormView = ({ toggleStatus, setToggleStatus }: Props) => {
	...
	const handleEmploymentStatusCheck = (e: React.ChangeEvent<HTMLInputElement>) => {
		if (e.target.checked) {
			...
			setEmploymentStatus([...employment_status.filter((status) => status !== e.target.value)]);
		}
	};
							...
							<div className="w-2/6 mt-8 flex  items-center  select-none relative">
								{toggleStatus && (
									<ul className="absolute z-10  bg-[#FFFFFF] drop-shadow-xl  w-60 h-15 mt-80 pt-2 pl-4 pb-2 rounded">
										...
										{EMPLOYMENT_STATUS.map((status: string) => {
											return (
												<li
													className="flex flex-end items-center border-b-2 pb-2 pt-2"
													key={status}
												>
													<span className={`w-5/6 ${renderClassnameStatus(status)}`}>
														{renderStatus(status)}
													</span>
													<input
														...
														id={status}
														value={status}
														checked={employment_status.indexOf(status) !== -1 ? true : false}
										...
										>
											<Apply onClick={handleClickSecondApply} />
										</li>
									</ul>
								)}
