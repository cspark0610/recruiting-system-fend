...
const QskInterview: React.FC<QskInterviewProps> = ({ classes, videoCounter, isRecording }) => {
	...
	const postulations: IPostulation[] = candidateDetail.postulations!;

	console.log(postulations[0].videos_question_list, "aca video");
	/*TODO  VER COMO HACER ACA  videos_question_list llega vacio*/
	const renderVideoQuestionsList = () =>
		postulations.map((postulation) => (
			<div
				className={`${classes} progress-bar mobile:mx-5 laptop:mx-0 w-full`}
				key={postulation._id}
			>
				{postulation.videos_question_list!.map((question: IQuestion) => (
	...
	const renderVideoQuestionListOnePostulation = () => {
		return (
			<div className={`${classes} progress-bar mobile:mx-5 laptop:mx-0 w-full`}>
				{postulations[0].videos_question_list!.map((question: IQuestion) => (
					<div key={question.question_id} className={`step flex items-center`}>
						<div
							className={
								question.question_id === videoCounter
	...
	return <>{renderVideoQuestionListOnePostulation()}</>;
};
